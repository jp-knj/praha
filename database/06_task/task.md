...
1. トランザクションが満たすべき4つの特性とはなんでしょうか？
...
<details>
    <summary>回答</summary>
    ライブロックとは、あるユーザがあるリソースを使いたくて待っているのだが、他のユーザーがずっとリソースを確保していて、永遠に順番が回ってこないこと。

**具体例:**   
    ユーザAがテーブルXの全行を必要としているが  
    テーブルXは常に数百というユーザから更新を受けている。  
    なので、ユーザAはロックのかかってないページを１つとして見つけることができない。
    ユーザAはじっと座って全てのページが利用可能になるまで待つしかない。　　

**解決策:**   
DBA が関連するセッションをいくつか終了させ、ロールバックさせることはできる。
</details>

3. 占有ロックと共有ロックとはなんでしょうか？

<details>
    <summary>回答</summary>
    共有ロックとは、データベースシステムなどで記憶領域への同時アクセスを制限するロック機構の一つで、他の実行主体による参照は許可するが変更や排他ロックは制限する。

占有ロックとは、データベースシステムなどで記憶領域への同時アクセスを制限するロック機構の一つで、他の実行主体によるアクセスを完全に禁止するもの。  

ロックの方式のうち、他の主体による参照や書き込み、削除、ロックなどを完全に遮断し、処理の終了まで完全に独占するものを排他ロックという。対象を安全に更新したいときに行われる。同時に一つの主体しか排他ロックを行うことはできない。
一方、他の主体による参照は許可し、書き込みや排他ロックを禁じるという相違がある。
</details>